...
3. Optional Step for Headless Implementation : Comment or Remove `&& strpos($_SERVER['HTTP_ACCEPT'], 'text/html') !== false` condition in `app/bootstrap.php`
4. Run the following command: `bin/magento dev:profiler:enable '{"drivers":[{"type":"Holdenovi\\Profiler\\Driver\\Hierarchy"}]}'`
